...
const MyLists = (props) => {
  ...
  useEffect(() => {
    const fetchUserLists = async () => {
      ...
      const creatorQuerySnapshot = await db
        .collection("Lists")
        .where("listCreator.id", "==", activeUser.docId)
        .get();
      const collaboratorQuerySnapshot = await db
        .collection("Lists")
        .where("listUserIds", "array-contains", { id: activeUser.docId })
        .get();
      const listsData = [];
    ...
    <div style={{ height: "100%" }}>
      {userLists.length > 0 ? (
        userLists.map((list, index) => {
          const currentUser = list.listUsers.find(
            (user) => user.id === activeUser.docId
          );
          const disableEdit =
            currentUser && currentUser.listFriendRole === "Reader";
          return (
                  ...
                  >
                    ...
                    >
                      ...
                      <Button
                        variant="contained"
                        color="error"
                        onClick={() => handleDeleteConfirmation(list)}
                        disabled={activeUser.docId !== list.listCreator.id}
                      >
                        Delete
                      </Button>
                    </div>
      ...
      <Dialog fullScreen open={showViewer} onClose={handleViewerClose}>
