...
const AddFriends = () => {
  ...
  useEffect(() => {
    if (activeUser.friends !== undefined) {
      const filteredResults = activeUser.friends.filter(
        (user) =>
          user.friendFirstName
            .toLowerCase()
  ...
  const handleAddListUser = (w, x, y, z) => {
    ...
    const newListUserIds = [...initListInfo.listUserIds, { id: x }];
          ...
          >
            ...
            >
              <Avatar
                ...
                {...user.friendAvatar}
              />
              {user.friendFirstName.charAt(0).toUpperCase() +
                user.friendFirstName.slice(1) +
                " " +
                user.friendLastName.charAt(0).toUpperCase() +
                user.friendLastName.slice(1)}
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              {initListInfo.listUsers.some(
                (listUser) => listUser.id === user.friendDocId
              ) ? (
