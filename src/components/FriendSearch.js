  ...
  const handleUserSearch = async () => {
    try {
      ...
      const filteredResults = results.filter(
        (result) => result.docId !== activeUser.docId
      );
        ...
        InputProps={{
          endAdornment: (
            <InputAdornment position="end">
              <IconButton edge="end" onClick={handleUserSearch}>
                <SearchIcon />
              </IconButton>
            </InputAdornment>
          ),
        ...
        <Collapse in={searchResults.length > 0}>
          {searchResults.map((result, index) => {
            const isFriend = userFriends.some(
              (friend) => friend.friendDocId === result.docId
            );
            return (
              <div
                key={index}
              ...
              >
                ...
                >
                  <Tooltip
                    title={isFriend ? "Friend Already Requested" : "Add Friend"}
                  >
                    <IconButton
                      edge="end"
                      color="primary"
